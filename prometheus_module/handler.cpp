...
namespace prometheus_module {
	namespace detail {
		...
		bool MetricsHandler::handleGet(CivetServer*, struct mg_connection* conn) {
			...
			request_latencies_.Observe((double)duration.count());

			bytes_transferred_.Increment((double)bodySize);
			num_scrapes_.Increment();
			return true;
		}
		std::vector<prometheus::MetricFamily> MetricsHandler::CollectMetrics() const {
